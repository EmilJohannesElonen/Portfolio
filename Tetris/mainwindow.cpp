...
{
    if( !game_on_ or active_tetromino_.size() == 0) {
        return;
    }
...
int MainWindow::get_difficulty()
{
    ...
    if( ui->easyRadioButton->isChecked() or
                                  ...
                                  bool use_active )
{
    ...
    for( auto coord : new_coords ) {
            if( 0 > coord.x or coord.x >= BORDER_RIGHT ) {
                possible = false;
...
void MainWindow::sort_highscores()
{
    ...
    for( it = sorting_map.begin(); it != sorting_map.end(); it++ ) {
        ...
        for( int a = it->second.size(); a > 0; a--) {
...
void MainWindow::move_tetromino_down()
{
    if( !is_move_possible(zero_move_,step_move_,true) ) {
        ...
        if( is_progressive_ ) {
            if( blocks_placed_%10 == 0 and current_level_ > 1) {
                current_level_ -= 1;
                ui->levelLcdNumber->display(11-current_level_);
    ...
    if( not file ) {
        ...
        while( std::getline(file,row) ) {
            ...
            if( data.size() == 3 ) {
                ...
                if( index.toInt() < 6 and index.toInt() > 0 ) {
                    bool numeric = false;
                    score.toInt(&numeric);
                    if ( numeric ) {
